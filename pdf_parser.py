...
class PDFParser():
    ...
    def __init__(self):
        ...
        self.models = {form_name: model_utils.load_form_model(
            form_name) for form_name in self.forms}
    ...
    def form_details(self, form_name):
        ...
        for field in sorted(fields.values(), key=lambda x: x['idx']):
            details.append({'id': field['id'], 'question': field['name'], 'type': field['type']
                            if field['type'] != 'char' else 'text', 'options': field['options']})
    ...
    def fill_form(self, form_name, answers):
        ...
        pointer_values = answer_utils.parse_answers(
            answers, self.models[form_name])
    ...
    def check_form_exists(self, form_name):
        if form_name not in self.forms:
            print('Requested form \'%s\' not in available forms. Try one of: %s' % (
                form_name, str(self.forms)))
            return False
